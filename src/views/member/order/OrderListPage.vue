<template>
    ...
    <div class="member-order">
      ...
      <div class="order-list" v-if="orderList">
        ...
        <div class="order-list" v-if="orderList && !loading">
          <OrderItem v-for="item in orderList.items" :key="item.id" :order="item" @onCancelOrder="onCancelOrderHandler"
            @onViewLogistics="onViewLogisticsHandler" @onReloadOrderList="getData"></OrderItem>
        </div>
  ...
  <CancelOrder ref="cancelOrderComponent" @onReloadOrderList="getData"></CancelOrder>
  <!-- 查看物流信息 -->
